...
static unsigned int tdt_data_rcvd(char* msgptr, char* signature, char masterNode)
{
    ...
    if (pktType == 254) {
        ...
        printf("******** Num of ACK: %d. Received Dedicated ack: 0x%x, 0x%x, 0x%x. ********\r\n", msgptr[10], msgptr[11], msgptr[12], msgptr[13]);
    ...
    if (localAddr == Master_node) {
        ...
        for (i = 0; i < ADDRESS_BOOK_LEN; i ++) {
            if (*ptraddr != 0) {
                ...
                ui_err = tda_remSendData(XID, *ptraddr, message, 4);
                ++ ptraddr;
                Com_Sleep(8);
            }
            else {
...
unsigned int tdt_txTask(void)
{
    ...
    if ((transmit_now == 1) && (do_not_transmit == 0)) {
        ...
        if (transmit_from_buffer == 1) {
            ...
            com_flag = 1;
            printf("Counter: 0x%x. sub-payload is 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x, 0x%x.\r\n", message[0], timePktSent[0], timePktSent[1], Qempty, TarsITx, acks[0], acks[1], acks[2]);
            ...
            printf("Counter: 0x%x, TarsITx: 0x%x, ACKs: 0x%x, 0x%x, 0x%x.\r\n", message[0], TarsITx, acks[0], acks[1], acks[2]);
#if 0
            // Update the time table
            ...
            if (acks[0] != 0) {
                printf("Transmit dedicated ACKs: 0x%x, 0x%x, 0x%x, 0x%x, 0x%x.\r\n", acks[0], acks[1], acks[2], acks[3], acks[4]);
                memset(message, 0, sizeof(message));
                msgptr = message;
                msgptr[0] = 253;
                msgptr[1] = 0;
...
/////////////////////////////////////////////////////////////////////////
//
// Function Name:	tdt_dataCopy
//
// Description:		Copy data monitored to other thread...
//
// Parameter:
//        Input:
...
//
////////////////////////////////////////////////////////////////////////
unsigned int tdt_dataCopy(char* data)
{
    char* ptr;
    char nodeAddr;
    int i, converter;
    ...
    hObject_StateTbl[3] = Q_empty[1].in;
    hObject_StateTbl[4] = Q_empty[0].out;
    ...
    if (data != NULL) {
        ...
        ptr += 4;
        for (i = 0; i < 8; i ++) {
            converter = (int)(hObject_StateTbl[i] * 10000);
            ptr[0] = (char)(converter >> 24);
            ptr[1] = (char)(converter >> 16);
            ptr[2] = (char)(converter >> 8);
            ptr[3] = (char)converter;
            ptr += 4;
        }
    }
