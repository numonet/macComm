...
#define GENERAL_PAYLOAD_SIZE		16
...
static unsigned int tdt_addAddrToBook(char* dataPacket, char* callFrom)
{
    ...
    if (strncmp(callFrom, "Ping", 4) == 0) {
        ...
        ptrNum = dataPacket;
        address = ((int)ptrNum[0] << 8) + (int)ptrNum[1];
        tdt_getModemByAddr(address, &addrIndex);
        if (addrIndex == -1) {
            bookptr = &Addr_book[0];
            for (j = 0; j < ADDRESS_BOOK_LEN; j ++) {
...
static unsigned int tdt_data_rcvd(char* msgptr, char* signature, char masterNode)
{
    ...
    char pktType, counter, sender, packetno;
    char timeRcv[2], clock_diff_msg[4];
    int i, t1_num, t2_num, t3_num, t4_num, quant, offset, addrIndex, packet;
    ...
    if (pktType == 254) {
        ...
        if (addrIndex != -1) {
            ...
            printf("	Transmission took: %ds and %ds in Tx.\r\n", timeDelay_total, timeDelay_Tx);


            for (i = 0; i < 3; i ++) {
                // remove the packet from buffer queue
                if (msgptr[18 + i] != 0) {
                    for (packet = 0; packet < Tx_queue[addrIndex].Buffer_len; packet ++) {
...
//
////////////////////////////////////////////////////////////////////////
unsigned int tdt_modemInit(int macProtocol)
{
    ...
    if (Master_node == localAddr) {
        state.Ping = 10;
    }

    // Create log file
    Com_fileInit("/root/log.txt");
    // Clear Uart receiving buffer
    Com_ClearRbuffer();
...
unsigned int tdt_txTask(void)
{
    ...
    int i, j, record_num, packet_addr, offset, temp, roffset;
    float probability, time_elapsed_mod;
    char record[ADDRESS_BOOK_LEN], time_char[2];
    char transmit_now, transmit_from_buffer, timePktSent[2];
    char com_flag, transmit_from_queue, do_not_transmit;
    char xid_, addr_, Qempty, TarsITx, acks[1][3], message[32];
    char* msgptr;
    ...
    if((state.Tx == 0) && (state.Ping == 0) && (probability < optNodeProb)) {
        ...
        for (i = 0; i < ADDRESS_BOOK_LEN; i ++) {
            if (Tx_queue[i].Buffer_len > 0) {
                time_waited = (time_now - Tx_queue[i].Buffer_time[0]) % 3600;
                if (time_waited > TARS_retrans_timeout) {
                    ...
                    if (mac_protocol == 1) { // ALOHA
                        ...
                        time_elapsed = (time_now - last_packet_TX_time) % 3600;
    ...
    if ((transmit_now == 1) && (do_not_transmit == 0)) {
        ...
        // prepare the data packet
        for (i = 0; i < 3; i ++) {
            if (Rx_queue[packet_addr].length > 0) {
                // if there is an ack, include it, and remove it from queue
                for (j = 0; j < ADDRESS_BOOK_LEN; j ++) {
                    if (Rx_queue[packet_addr].PacketList[j] != 0) {
                        acks[0][i] = Rx_queue[packet_addr].PacketList[j];
                        Rx_queue[packet_addr].PacketList[j] = 0;
                        Rx_queue[packet_addr].length -= 1;
                        break;
                    }
                }
            }
        ...
        if (transmit_from_buffer == 1) {
            ...
            msgptr = message;
            for (i = 0; i < 32; i ++) {
                if (Tx_queue[packet_addr].Buffer_flag[i] != 0) {
                    memcpy(msgptr, Tx_queue[packet_addr].Buffer_payload[i], GENERAL_PAYLOAD_SIZE);
                    break;
                }
            }
            ...
            xid_ = XID;
            addr_ = Tx_queue[packet_addr].address;
            com_flag = 1;
            //tda_remSendData(XID, Tx_queue[packet_addr].address, message, msglen);
            Com_Printf("Tx_queue, is being transmitted to ...\r\n", 5);
            ...
            if (Tx_queue[packet_addr].Buffer_Retries[0] < 2) {
                for (j = 0; j < 32; j ++) {
                    if (Tx_queue[packet_addr].Buffer_flag[32 - j - 1] == 0) {
                        break;
                    }
                }
                memcpy(Tx_queue[packet_addr].Buffer_payload[32 - j - 1], Tx_queue[packet_addr].Buffer_payload[j], GENERAL_PAYLOAD_SIZE);
                Tx_queue[packet_addr].Buffer_time[32 - j - 1] = Packet_sent_at;
                Tx_queue[packet_addr].Buffer_Retries[32 - j - 1] = Tx_queue[packet_addr].Buffer_Retries[i] + 1;
                Tx_queue[packet_addr].Buffer_flag[32 - j - 1] = 1;
            }
            else {
                Tx_queue[packet_addr].Buffer_len -= 1;
            }
            // Remove from Buffer:
            Tx_queue[packet_addr].Buffer_time[i] = 0;
            Tx_queue[packet_addr].Buffer_Retries[i] = 0;
            Tx_queue[packet_addr].Buffer_flag[i] = 0;
            ...
            memset(message, 0, sizeof(message));
            msgptr = message;
            roffset = Tx_queue[packet_addr].roffset;
            memcpy(msgptr, Tx_queue[packet_addr].payload[roffset], GENERAL_PAYLOAD_SIZE);
            msgptr += GENERAL_PAYLOAD_SIZE;
            msgptr[0] = timePktSent[0];
            msgptr[1] = timePktSent[1];
            msgptr[2] = Qempty;
            ...
            com_flag = 1;
            //tda_remSendData(XID, Tx_queue[packet_addr].address, message, msglen);
            Com_Printf("Tx_queue payload, is being transmitted to ...\r\n", 5);
            // Update the time table
            ...
            // Add to buffer:
            for (j = 0; j < 32; j ++) {
                if (Tx_queue[packet_addr].Buffer_flag[j] == 0) {
                    Tx_queue[packet_addr].Buffer_len += 1;
                    memcpy(Tx_queue[packet_addr].Buffer_payload[j], Tx_queue[packet_addr].payload[roffset], GENERAL_PAYLOAD_SIZE);
                    Tx_queue[packet_addr].Buffer_time[j] = Packet_sent_at;
                    Tx_queue[packet_addr].Buffer_Retries[j] = 0;
                    Tx_queue[packet_addr].Buffer_flag[j] = 1;
                    break;
                }
            }
            // Update read pointer for payload buffer
            if (roffset == 32 - 1) {
                Tx_queue[packet_addr].roffset = 0;
            }
            else {
                Tx_queue[packet_addr].roffset += 1;
            }
            // ...
            queue_length.Tx -= 1;
            // Tx packet started
...
unsigned int tdt_notifyTask(void)
{
    ...
    if (tda_asyncNotify(pmPkt) == 0) {
        // Write the packet to log file
        tdt_notify_to_file(pmPkt, 1);
        for (i = 0; i < pmPkt->numParam; i ++) {
            // Generate Signature
            if (pmPkt->len[i] >= 4) {
                sprintf(signature, "%x %x %x %x %x %x", pmPkt->subsys[i], pmPkt->field[i], 
    ...
    for (i = 0; i < ADDRESS_BOOK_LEN; i ++) {
        if (Addr_book[i].address != 0) {
            ...
            if (pTxqueue->length > 0) {
                ...
                if (((numClock - pTxqueue->Buffer_time[0]) % 3600) > TARS_retrans_timeout) {
                    This_Q_empty = 0;
                }
            }
