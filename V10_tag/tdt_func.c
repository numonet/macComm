...
static unsigned int tdt_data_rcvd(char* msgptr, char* signature, char masterNode)
{
    ...
    int i, j, t1_num, t2_num, t3_num, t4_num, quant, offset, addrIndex, packet;
    ...
    if (pktType == 254) {
        ...
        if (addrIndex != -1) {
            ...
            //Rx_queue[addrIndex].length += 1;
            //for (i = 0; i < RXPACKET_LENGTH; i ++) {
            //    if ((Rx_queue[addrIndex].PacketList[i] == 0) && (Rx_queue[addrIndex].PacketList[i] != pktType)) {
            //        Rx_queue[addrIndex].PacketList[i] = pktType;
            //        break;
            //    }
            //}
...
unsigned int tdt_txTask(void)
{
    ...
    char xid_, addr_, Qempty, TarsITx, acks[3], message[32];
    char* msgptr;
    ...
    if((state.Tx == 0) && (state.Ping == 0) && (probability < optNodeProb)) {
        ...
        for (i = 0; i < ADDRESS_BOOK_LEN; i ++) {
            if (Tx_queue[i].Buffer_len > 0) {
                time_waited = (time_now - Tx_queue[i].Buffer_time[0]) % 36000;
    ...
    if ((transmit_now == 1) && (do_not_transmit == 0)) {
            ...
            if (Rx_queue[packet_addr].length > 0) {
                ...
                for (j = 0; j < RXPACKET_LENGTH; j ++) {
                    if (Rx_queue[packet_addr].PacketList[j] != 0) {
                        acks[i] = Rx_queue[packet_addr].PacketList[j];
        ...
        if (transmit_from_buffer == 1) {
            ...
            if (i < 32) {
                ...
                if (Tx_queue[packet_addr].Buffer_Retries[i] < 2) {
                    // temporarily store the info
                    temp_retries = Tx_queue[packet_addr].Buffer_Retries[i];
                    memcpy(temp_payload, Tx_queue[packet_addr].Buffer_payload[i], GENERAL_PAYLOAD_SIZE);
                    // Remove from Buffer
                    Tx_queue[packet_addr].Buffer_time[i] = 0;
                    Tx_queue[packet_addr].Buffer_Retries[i] = 0;
                    Tx_queue[packet_addr].Buffer_flag[i] = 0;
                    memset(Tx_queue[packet_addr].Buffer_payload[i], 0, 32);
                    // Re-arrange the Buffer array
                    for (j = i; j < (32 - 1); j ++) {
                        Tx_queue[packet_addr].Buffer_time[j] = Tx_queue[packet_addr].Buffer_time[j + 1];
                        Tx_queue[packet_addr].Buffer_Retries[j] = Tx_queue[packet_addr].Buffer_Retries[j + 1];
                        Tx_queue[packet_addr].Buffer_flag[j] = Tx_queue[packet_addr].Buffer_flag[j + 1];
                        memcpy(Tx_queue[packet_addr].Buffer_payload[j], Tx_queue[packet_addr].Buffer_payload[j + 1], GENERAL_PAYLOAD_SIZE);
                    }
                    Tx_queue[packet_addr].Buffer_flag[31] = 0;
                    ...
                    memcpy(Tx_queue[packet_addr].Buffer_payload[j], temp_payload, GENERAL_PAYLOAD_SIZE);
                    Tx_queue[packet_addr].Buffer_time[j] = Packet_sent_at;
                    Tx_queue[packet_addr].Buffer_Retries[j] = temp_retries + 1;
