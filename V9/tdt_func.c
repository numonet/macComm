...
unsigned int tdt_txTask(void)
{
    ...
    int i, j, record_num, packet_addr, offset, roffset;
    ...
    if ((transmit_now == 1) && (do_not_transmit == 0)) {
        ...
        if (transmit_from_buffer == 1) {
            ...
            if (Tx_queue[packet_addr].Buffer_Retries[0] < 2) {
                ...
                memcpy(Tx_queue[packet_addr].Buffer_payload[j], Tx_queue[packet_addr].Buffer_payload[i], GENERAL_PAYLOAD_SIZE);
                Tx_queue[packet_addr].Buffer_time[j] = Packet_sent_at;
                Tx_queue[packet_addr].Buffer_Retries[j] = Tx_queue[packet_addr].Buffer_Retries[i] + 1;
